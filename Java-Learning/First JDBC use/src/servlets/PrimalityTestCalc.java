...
public class PrimalityTestCalc extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try (PrintWriter out = response.getWriter()) {  
            ...
            String resString = testResults.get(0);                  // Need to prepare chain of strings separated by spaces to use DAO methods  
            String valString = testValues.get(0);
            for (int i = 1; i< testResults.size(); i++) {           
                resString = resString + " " + testResults.get(i);       
                valString = valString + " " + testValues.get(i);
            }  
            int wasAddedProperly = FermatTestDAO.addFermatTestUnit(testNumber, valString, resString);   // There we add unit to DB
            switch (wasAddedProperly) {
                case 1:
                    out.println("Added to history succesfully");
                    break;
                case -1:
                    out.println("Connection with database is not set, test was not added to history");
                    break;
                case 0:
                    out.println("Unexpected error occurred while adding to history, test may not be added properly");
                    break;
                case -2:
                    out.println("SQL Error occurred while adding test to history, test may not be added properly");
                default:
                    out.println("wtf");
                    break;
            }
